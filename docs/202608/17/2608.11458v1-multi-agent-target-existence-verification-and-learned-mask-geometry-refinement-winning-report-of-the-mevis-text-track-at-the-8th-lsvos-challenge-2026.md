...
title_zh: 多智能体目标存在性验证与学习式掩膜几何优化：第八届LSVOS挑战赛2026 MeViS-Text赛道获胜报告
...
tldr: 针对视频指代分割中的无目标表达易被误判为类别匹配掩码的问题，提出 SSUPER 流水线。其核心是多智能体多模态大模型分阶段独立推理并合成验证，同时将存在性验证解耦为完整时间谓词审计，区分缺失与暂时不可见并过滤相机运动干扰。该审计无需新增分割调用即可恢复大部分残余错误，配合纯数据驱动的掩码几何风格细化，最终以 0.9081339614 的分数夺得 MeViS-Text 赛道第一名。
source: arxiv
...
motivation: 硬负样本命名看似合理的对象，仅在完整时间谓词下失败，导致存在性误判，因此需将存在性验证解耦为独立审计。
method: SSUPER 使用多智能体多模态大模型分阶段执行提示并合成验证，并通过完整谓词审计区分缺失与遮挡，再以风格细化器对齐标注。
result: 在不新增分割调用的情况下恢复大部分残余无目标错误，最终得分为 0.9081339614，位列官方排行榜首位。
conclusion: 将存在性验证与分割解耦、语义与风格分离，可显著减少无目标误判，是视频指代分割的高效范式。
---

## 摘要
我们提出了第八届大规模视频目标分割（LSVOS）挑战赛2026 MeViS-Text赛道的第一名解决方案：基于书面运动表达引导的指代视频目标分割，包括具有欺骗性的无目标表达——这些表达与视频中的任何目标都不匹配，且必须在每一帧中产生空掩膜。我们的流程SSUPER将每个表达解析为视觉概念，使用SAM 3.1生成全视频候选掩膜片段，并选择目标ID。在每个推理阶段，三个异构多模态大语言模型独立执行相同的阶段特定提示，然后通过一次综合传递提交一个经模式验证的判定。尽管该系统在验证阶段拒绝了所有无目标表达，但排行榜显示，相当一部分测试无目标案例仍然漏过。原因在于，硬负样本命名了一个看似合理的目标，仅在完整的时间谓词下才会失败，因此当选择和存在性一起决定时，一个类别上合理的掩膜片段会锚定判定。因此，我们将存在性验证解耦为对完整谓词（类别、数量、动作、轨迹、事件顺序和语义角色）的独立多智能体审计，该审计区分缺失与暂时不可见，折扣由相机运动引起的表观运动，并要求矛盾证据而非仅仅不确定性来做出无目标判定。在不进行任何新的分割调用的情况下，该审计恢复了大部分残留的无目标错误。随后，一个仅使用训练数据的StyleRefiner将掩膜几何与MeViSv2的标注风格对齐，同时通过构造保留所有存在性判定，表明一旦语义固定，剩余误差的一部分是风格性的而非语义性的。完整系统在官方挑战排行榜上达到最终得分0.9081339614。
